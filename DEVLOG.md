...
This is a personal learning project. I'm
...
dev log gets updated after every "phase", including the real bugs I hit and
...
involving any other member in it.
...
it's mostly one of two things — the database isn't running, or it
...
different order than I wrote them. Purely cosmetic (i think)
...
opposite direction of an unpaid debt. `GET /api/users/me/balances` to display balances across all networks for the user.
...
**Frontend:**
Added feature to show pairwise balances on NetworkDetailsPage.tsx. Instead of showing multiple seperate balances between two users, it shows one collapsed balance (backend derives the balance every time)

### Phase 6: Settlements
Added `POST /api/networks/{id}/settlements` endpoint to backend that allows users to record settlements. Added `SettlementResponse`, `SettlementService.getSettlementsForNetwork()`,
and `GET /api/networks/{id}/settlements`. On the frontend: `SettleUpForm.tsx`
to record a payment, `SettlementHistory.tsx` for a payer → payee timeline.

**Manual tests performed:**
- Partial settlement — paid less than the full balance, remaining amount
  came back correct ([$X owed, paid $Y, $Z remaining — fill in real numbers])
- Overpayment — paid more than owed, direction flipped correctly and the
  new amount owed in the other direction was right ([owed $X, paid $Y,
  now owed $Z back — fill in real numbers])
...
Started writing real unit tests for `BalanceService` — JUnit5 + Mockito,
every repository mocked out so what's actually being tested is the
netting logic itself, not whether JPA/Postgres work.

**Tests so far:**
- Two opposing purchases between the same pair net to a single correct
  balance
- A settlement matching an existing balance zeroes it out — this test is
  what formally caught the sign bug from Phase 5. Failed against the old
  code (expected 0 balances, got 1 with the doubled amount), passes now
- A self-purchase (recipient = purchaser) produces no balance entries

**Note:** never compare `BigDecimal` with `==` or `.equals()`
for equality — `.equals()` also checks scale, so `2.0` and `2.00` count
as "not equal." Use `.compareTo(...) == 0`.

**Still to do:**
- Automate partial-settlement and overpayment cases
- Three-person-chain test (confirm unrelated pairs stay independent)
- Soft-deleted-purchase test
- Basic `PurchaseService` coverage — multi-item creation, recipient
  membership validation

  ### Phase 7: Rate Limiting

Added a `RateLimitFilter` (Bucket4j, in-memory, per IP) ahead of
`JwtAuthenticationFilter`, capping `/api/auth/**` to 10 requests/minute —
mainly so brute-forcing a login is impractical. Known limitation: resets on restart, wouldn't hold up correctly
across multiple instances if this ever scaled horizontally.

### Phase 7: Deployment

Backend on Railway (Spring Boot + Postgres), frontend on Vercel. Made
every environment-specific value — DB connection, JWT secret, CORS
origin — read from env vars with local-dev fallbacks
(`${VAR_NAME:localDefault}`), same idea on the frontend with Vite's
`VITE_`-prefixed vars.

**Bugs/gotchas encountered:**

1. Railway's `DATABASE_URL` isn't directly usable as
   `spring.datasource.url` — it's `postgresql://user:pass@host/db`, but
   Spring needs `jdbc:postgresql://...` with credentials as separate
   properties. Built the JDBC URL manually from `PGHOST`/`PGPORT`/
   `PGDATABASE` instead.

2. CORS — backend needs the frontend's real URL, frontend
   needs the backend's real URL. Deployed both once, then went back and
   set `APP_CORS_ALLOWED_ORIGIN` on Railway to the real Vercel URL as a
   last step.

**Manual tests performed:**
- Full flow against production: register, login, create/join a network,
  log a multi-item purchase, check balances, record a settlement — all
  working against the real deployed DB

### Phase 7: UI Polish

Used Cursor AI Agents with a scoped prompt (styling/CSS only, no logic/hooks/props/
API/state changes) to restyle the frontend, which had basically no
styling up to this point. CSS Modules + a shared design-token
stylesheet, no new dependencies. Reviewed the plan and every component's
diff before accepting anything.